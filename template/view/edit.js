...
const templateTableColumn = (props) => {
  ...
  for (key in column) {
    ...
    <zt-col :span="${column[key].span || 24}">
      <zt-form-item :label="$t('${props.project}_${column[key].$t || column[key].prop}')" prop="${column[key].prop}" ${column[key].rules?':rules="[rules.'+column[key].prop+']"':''}  ${!column[key].rules && column[key].required?':rules="[rules.'+(column[key].inputType==='select'?'select':'input')+']"':''}>
